...
leveraging computer vision to recreate Harry's most treasured possession. 
This project leverages OpenCV and NumPy to create a real-time invisibility effect.

## ⚡ Features ⚡

- **Real-time Tracking**: Continuously processes your webcam, revealing and concealing objects.
- **Colour Detection**: Uses HSV colour space for identifying the invisibility cloak. 
- **Mask Creation**: Creates precise masks to separate the cloak from the scene.
...
dependencies, or start from scratch by installing the dependencies and working through the code:
...
def create_mask(frame, lower_color, upper_color):
    # Convert frame to HSV colour space for better colour detection
